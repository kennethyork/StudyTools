...
| **Bible Dictionary** (`apps/dictionary/`) | Six sources together: Easton's (1897), Smith's (1863) and Hastings' (1909) on the Bible's people, places and subjects; Hitchcock's Bible Names (1869) for what a name means; the whole of Webster's 1828 Dictionary, 60,968 entries, which is where *lasciviousness* and the King James sense of *conversation* are defined, with the words the King James uses marked and shown first — and, where those leave a gap a reader meets often, a note written for this site quoting the verse it comes from. A word the King James writes in an older form (*aboundeth*, *pouredst*, *oxen*) is found under its headword. Scripture citations link straight into the Verse Matrix. |
...
| **Atlas** (`apps/atlas/`) | Every place named in the Bible, on a map that ships with the site rather than being fetched: the ground is Natural Earth's 1:50m shaded-relief raster (public domain), cropped to the Bible lands and reduced to one JPEG, the coastlines, rivers, lakes and today's borders are Natural Earth 1:50m as GeoJSON, the 1,335 places with a position are the OpenBible.info geocoding dataset (CC BY 4.0, the same publisher as the cross-references), and each place carries the verses it appears in, opening in the reader. Pan by dragging (the map moves as a picture, and is redrawn once when you let go), zoom with the wheel, the buttons or the plus and minus keys, nudge it with the arrow keys, search by a name, follow a journey stop by stop, or narrow the map to the places one book and chapter names. A place's dot is as large as how often the Bible names it, the ground fades rather than vanishing as you zoom past what the raster resolves, and a scale bar in kilometres is drawn on the map. No tiles and no tile server, so nothing learns what you are looking at; the map shows where the dataset puts each place today, never a political line of any biblical period — the checkbox for today's borders starts off — and says so. |
| **Commentary** (`apps/commentary/`) | The commentary on a whole chapter rather than one verse: every remark on the chapter in order, each attributed, with the verse text above it. Narrow it to one work, or — for the Catena — to one of the Fathers, and read that writer through the chapter. Printable, and each verse links to Study a Passage and to Ask the Site. All of it is read from the same chapter files the reader loads; nothing is fetched from anywhere else. |
| **Ask the Site** (`apps/ask/`) | The other half of the reader's Ask panel, with no model in it. A question becomes a plan of lookups over what this site already holds — commentary, dictionaries, cross-references, topical Bibles, the interlinear, the concordance — and the answer is what those files say, each part cited to its source. A reference returns the text in several translations, the commentators, the cross-references, the Hebrew or Greek and where the Prayer Book reads it; a word returns the dictionary entries and the topical subjects; a Strong's number returns the word, its gloss and every verse it appears in; "compare …" returns every translation. It cannot reason or summarise, and where the files have nothing it says so and offers what it can answer. |
...
data/atlas/              the atlas: places with their verses, the layers to draw,
                         the journeys, and relief.jpg, the ground itself
data/dictionary/         Easton, Smith, Hastings, Hitchcock, the whole 1828, and notes (33 MB)
...
node scripts/check-dictionary.js      # re-checks the six sources, merged, with the King James' words marked
...
python3 scripts/build-atlas.py        # the places of the Bible, and a map to put them on
node scripts/check-atlas.js           # re-checks those places, their verses, and the ground they stand on
...
| Atlas places and their verses | [OpenBible.info Bible Geocoding](https://github.com/openbibleinfo/Bible-Geocoding-Data) | CC BY 4.0 (its own sources are cited in the dataset; some identifications are contested) |
| Atlas ground (the basemap) | [Natural Earth](https://www.naturalearthdata.com/) 1:50m raster `HYP_50M_SR_W`, hypsometric tints with shaded relief and water, via [nvkelso/natural-earth-raster](https://github.com/nvkelso/natural-earth-raster) | Public domain |
| Atlas coastlines, lakes, rivers, borders and cities | [Natural Earth](https://www.naturalearthdata.com/) 1:50m (1:110m for the world view), via [martynafford/natural-earth-geojson](https://github.com/martynafford/natural-earth-geojson) | Public domain (CC0) |
...
| Bible dictionaries: Easton, Smith, Hastings, Hitchcock | [NEUU bible-dictionary-dataset](https://github.com/neuu-org/bible-dictionary-dataset) (CCEL ThML) | CC BY 4.0; source texts public domain |
| Dictionary notes written for this site | the text of the King James, quoted verse by verse, checked by `check-dictionary.js` | This project's own writing; the quotations are the public-domain text |
| Bible dictionary: Webster's 1828, the whole of it (60,968 entries) | [a transcription of the 1828](https://github.com/DataWar/1828-dictionary) (MIT repository) with the word list taken from the KJV at [eBible.org](https://ebible.org/Scriptures/eng-kjv_usfm.zip) | Public domain (the dictionary is 1828, Webster died 1843; the KJV is public domain). 2,004 rows of the transcription are its own error pages and are left out |
