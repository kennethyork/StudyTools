...
| **Atlas** (`apps/atlas/`) | Every place named in the Bible, on a map that is drawn rather than fetched: the coastline, lakes and rivers are Natural Earth 1:110m (public domain) shipped as GeoJSON, the 1,335 places with a position are the OpenBible.info geocoding dataset (CC BY 4.0, the same publisher as the cross-references), and each place carries the verses it appears in, opening in the reader. Pan by dragging, zoom with the wheel, search by name. No tiles and no tile server, so nothing learns what you are looking at; the base map shows where the dataset puts each place today and the borders of no period at all, which the page says. |
...
python3 scripts/build-atlas.py        # the places of the Bible, and a coastline to put them on
...
| Atlas coastline, lakes and rivers | [Natural Earth](https://www.naturalearthdata.com/) 1:110m, via [natural-earth-geojson](https://github.com/martynafford/natural-earth-geojson) | Public domain (CC0) |
